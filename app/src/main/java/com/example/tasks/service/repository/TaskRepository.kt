...
class TaskRepository(val context: Context): BaseRepository(context) {
    ...
    //PODE SER FEITO EM UMA CASE OU CHAMAR APENAS O call: Call<List<TaskModel>> NO EXEMPLO DA "private fun list(call: Call<List<TaskModel>>, listener: APIListener<List<TaskModel>>){"
    /*fun all(listener: APIListener<List<TaskModel>>){
        val call: Call<List<TaskModel>> = mRemote.all()
